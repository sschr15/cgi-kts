...
internal object CgiScriptCompilationConfiguration : ScriptCompilationConfiguration({
    ...
    private fun handleMavenCentral(context: ScriptConfigurationRefinementContext): ResultWithDiagnostics<ScriptCompilationConfiguration> {
        val source = context.collectedData?.get(ScriptCollectedData.collectedAnnotations)
            ?.firstOrNull { it.annotation is DependsOnMavenCentral }
            ?: return context.compilationConfiguration.asSuccess()

        val mvnCentral = source.annotation as DependsOnMavenCentral

        data class Dep(val group: String, val artifact: String, val version: String, val classifier: String, val extension: String)
        val dep = if (mvnCentral.coordinates.isNotBlank()) {
            val match = artifactRegex.matchEntire(mvnCentral.coordinates) ?: return fail("Invalid coordinates: ${mvnCentral.coordinates}", source.location)
            ...
            if (mvnCentral.artifact.isBlank()) return fail("No specified MavenCentral artifact", source.location)
        ...
        return if (result.isSuccess) {
            ...
            fail("Failed to download artifact from $url", source.location, result.exceptionOrNull() ?: NullPointerException("result.exceptionOrNull() returned null despite the result not being a success"))
        }
    }

    private fun handleFile(context: ScriptConfigurationRefinementContext): ResultWithDiagnostics<ScriptCompilationConfiguration> {
        val source = context.collectedData?.get(ScriptCollectedData.collectedAnnotations)
            ?.firstOrNull { it.annotation is DependsOnLocal }
            ?: return context.compilationConfiguration.asSuccess()
    ...
    private fun handleUrl(context: ScriptConfigurationRefinementContext): ResultWithDiagnostics<ScriptCompilationConfiguration> {
        val source = context.collectedData?.get(ScriptCollectedData.collectedAnnotations)
            ?.firstOrNull { it.annotation is DependsOnUrl }
            ?: return context.compilationConfiguration.asSuccess()
        ...
        return if (result.isSuccess) {
            ...
            fail("Failed to download artifact from $url", source.location, result.exceptionOrNull())
        }
    }
